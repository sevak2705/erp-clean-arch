...
namespace CleanArchitectureApp.Application.Features.Contact.Handlers
{
    public class CreateCustomerContactCommandHandler(
        ...
        IMapper mapper
    ) : IRequestHandler<CreateCustomerContactCommand, Unit>
    {
        ...
        {
            // basic validation
            var validationResult = await new CreateCustomerContactCommandValidator().ValidateAsync(
                request,
                cancellationToken
            );
            ...
            var isContactExist = await _customerContactRepository.GetContactByPhoneAsync(
                request.PhoneNo
            );
